...
def get_all_repo_files(repo,project="Quantum",organization="EntDataProd"):
    ...
    pat_token = "7lqkoguaodvtyi24xwx4as5ofljstst242uic34ype255e4gkxgq"
...
def get_files_with_txt(repo,search_text,project="Quantum",organization="EntDataProd"):
  ...
  files_with_text_meta = [[repo,file_path[file_path.rfind("/")+1:]]for file_path in code_files_path if search_text in get_code(repo,file_path)]
...
repos = ["curated","semantic_bi","semantic_ds","semantic_ca"]
search_text = "/mnt/"
search_results = []
...
search_results_spark = spark.createDataFrame(pd.DataFrame(search_results,columns=["RepoName","NotebookName"]))
